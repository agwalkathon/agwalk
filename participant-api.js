...
async function load(isBackgroundRefresh) {
  ...
  loadNotifications();
  if (typeof loadParticipantCertificates === 'function') loadParticipantCertificates(s, athleteId);
  try {
...
window.loadParticipantCertificates = async function(reg, athleteId) {
  var certSec = document.getElementById('you-certificates-section');
  var certCard = document.getElementById('you-certificates-card');
  if (!certSec || !certCard || !reg || !reg.email) return;

  try {
    var res = await fetch(SUPABASE_URL + '/rest/v1/registration?email=eq.' + encodeURIComponent(reg.email) + '&event_id=eq.1&select=event_id,event_name,leaderboard_team,gender,shift,strava_athlete_id', { headers: HDR });
    var walkRegs = await res.json();
    if (!Array.isArray(walkRegs) || walkRegs.length === 0) return;
    
    var wReg = walkRegs[0];
    var totalKm = 0;
    
    try {
      var cacheRes = await fetch(SUPABASE_URL + '/rest/v1/athlete_points_summary?athlete_id=eq.' + wReg.strava_athlete_id + '&event_id=eq.1', { headers: HDR });
      var cacheData = await cacheRes.json();
      if (Array.isArray(cacheData) && cacheData.length > 0) {
        totalKm = parseFloat(cacheData[0].total_distance_km || 0);
      }
    } catch(e) {}

    var isFemale = (wReg.gender || '').toLowerCase() === 'female';
    var goldThresh = isFemale ? 250 : 300;
    var silverThresh = isFemale ? 150 : 200;
    var bronzeThresh = isFemale ? 100 : 125;
    ...
    window.walkathonCertData = {
      name: reg.name || reg.full_name || 'Participant',
      medal: medalTitle
    };
    ...
    console.error('Failed to load certificates section:', ex);
  }
};
